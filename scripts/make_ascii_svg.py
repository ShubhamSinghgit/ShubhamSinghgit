...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">{html.escape(os.environ.get("GH_PROFILE_USER","avi"))}@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{html.escape(os.environ.get("GH_PROFILE_USER","avi"))}@github:~$ whoami <tspan fill="{INK}">{html.escape(os.environ.get("PROFILE_NAME") or os.environ.get("GH_PROFILE_USER","YOUR NAME"))}</tspan></text>')
